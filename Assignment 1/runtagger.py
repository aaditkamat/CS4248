...
def preprocess(lines, model):
    ...
    model['emission_probs'] = smoothen(
        word_counts, bigram_counts, emission_probs)
...
def viterbi(model, line):
    ...
    for tag in tags:
        viterbi_vals[(tag, words[0])] = transition_probs[(
            START_TOKEN, tag)] * emission_probs[(tag, words[0])]

    for i in range(1, len(words)):
        word1, word2 = words[i - 1: i + 1]
        for tag2 in tags:
            viterbi_vals[(tag2, word2)] = max(
                [viterbi_vals[(tag1, word1)] * transition_probs[(tag1, tag2)]
                    * emission_probs[(tag2, word2)] for tag1 in tags]
            )
