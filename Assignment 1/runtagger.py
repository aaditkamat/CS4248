...
def smoothen(unigram_counts, bigram_counts, bigram_probs):
    bigram_probs = {bigram: bigram_count/ (unigram_counts[bigram[0]]) for bigram, bigram_count in bigram_counts.items()}
...
def viterbi(model, line):
    ...
    for tag in tags:
        viterbi_vals[(tag, words[0])] = transition_probs[(START_TOKEN, tag)] * emission_probs[(tag, words[0])]

    for i in range(1, len(words)):
        word1, word2 = words[i - 1: i + 1]
        for tag2 in tags:          
            viterbi_vals[(tag2, word2)] = max([viterbi_vals[(tag1, word1)] * transition_probs[(tag1, tag2)] * emission_probs[(tag2, word2)] for tag1 in tags])

    viterbi_vals[(END_TOKEN, words[-1])] = max([viterbi_vals[(tag, words[-1])] * transition_probs[(tag, END_TOKEN)] for tag in tags])
